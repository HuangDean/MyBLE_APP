package com.sunyeh.m100

import android.app.Application
import android.content.Context
import android.util.Log
import com.clj.fastble.BleManager
import com.clj.fastble.callback.BleWriteCallback
import com.clj.fastble.data.BleDevice
import com.clj.fastble.utils.HexUtil
import org.koin.android.ext.koin.androidContext
import org.koin.core.context.startKoin

class MyApplication : Application() {

    companion object {
        lateinit var context: Context

        const val SERVICE_UUID: String = "6E400001-B5A3-F393-E0A9-E50E24DCCA9E"
        const val CHARACTERISTIC_UUID_WRITE: String = "6E400002-B5A3-F393-E0A9-E50E24DCCA9E"
        const val CHARACTERISTIC_UUID_NOTIFY: String = "6E400003-B5A3-F393-E0A9-E50E24DCCA9E"

        var bleDevice: BleDevice? = null

        var delegate: NotifyDelegate? = null

        // 每次寫入建議等300ms，建議不要再UI Thread
        fun writeData(cmd: ByteArray, callback: BleWriteCallback, isWrite: Boolean) {
            if (bleDevice == null) return
            if (!BleManager.getInstance().isConnected(bleDevice)) return

            var cmdBytes: ByteArray = if (isWrite) {
                byteArrayOf(0x01, 0x10, *cmd)
            } else {
                byteArrayOf(0x01, 0x03, *cmd)
            }

            cmdBytes = byteArrayOf(*cmdBytes, *getCRC(cmdBytes))
            Log.e("write cmd", HexUtil.formatHexString(cmdBytes, true))

            BleManager
                .getInstance()
                .write(bleDevice, SERVICE_UUID, CHARACTERISTIC_UUID_WRITE, cmdBytes, false, callback)
        }

        fun writeData(cmd: ByteArray, callback: BleWriteCallback, isWrite: Boolean, no: String) {
            if (bleDevice == null) return
            if (!BleManager.getInstance().isConnected(bleDevice)) return

            var cmdBytes: ByteArray = if (isWrite) {
                byteArrayOf(0x01, 0x10, *cmd)
            } else {
                byteArrayOf(0x01, 0x03, *cmd)
            }

            cmdBytes = byteArrayOf(*cmdBytes, *getCRC(cmdBytes))
            Log.e("write cmd $no", HexUtil.formatHexString(cmdBytes, true))

            BleManager
                .getInstance()
                .write(bleDevice, SERVICE_UUID, CHARACTERISTIC_UUID_WRITE, cmdBytes, false, callback)
        }

        fun writeData(cmd: ByteArray, callback: BleWriteCallback, msg: String) {
            if (bleDevice == null) return
            if (!BleManager.getInstance().isConnected(bleDevice)) return

            Log.e("write cmd $msg", HexUtil.formatHexString(cmd, true))

            BleManager
                .getInstance()
                .write(bleDevice, SERVICE_UUID, CHARACTERISTIC_UUID_WRITE, cmd, false, callback)
        }

        private fun getCRC(data: ByteArray): ByteArray {
            val crc: IntArray = intArrayOf(0xFF, 0xFF)
            var nextByte: Int
            var uIndex: Int

            /* pass through message buffer */
            var i = 0
            while (i < data.size && i < data.size) {
                nextByte = 0xFF and data[i].toInt()
                uIndex = crc[0] xor nextByte
                crc[0] = crc[1] xor auchCRCHi[uIndex].toInt()
                crc[1] = auchCRCLo[uIndex].toInt()
                i++
            }

            return intArray2ByteArray(crc)
        }


        private fun intArray2ByteArray(intArray: IntArray): ByteArray {
            return intArray.foldIndexed(ByteArray(intArray.size)) { i, a, v -> a.apply { set(i, v.toByte()) } }
        }

        /**
         * @return MODBUS CRC
         */

        private val auchCRCHi = shortArrayOf(
            
        )

        /* Table of CRC values for low-order byte */
        private val auchCRCLo = shortArrayOf(
            
        )
    }

    override fun onCreate() {
        super.onCreate()

        context = applicationContext

        val defaultUEH = Thread.getDefaultUncaughtExceptionHandler()
        if (defaultUEH !is CustomExceptionHandler) {
            Thread.setDefaultUncaughtExceptionHandler(CustomExceptionHandler(defaultUEH))
        }

        BleManager.getInstance().init(this)

        startKoin {
            androidContext(this@MyApplication)
            modules(mainModule)
        }


    }
}